---
name: jqdatasdk
description: 聚宽A股行情、历史K线、财务数据、指标数据等证券财经信息获取；当用户提及聚宽、jqdata、jqdatasdk，并且需要获取A股数据时使用
dependency:
  python:
    - jqdatasdk>=1.9.0
    - pandas>=1.3.0
...
- 脚本:
  ...
  - [scripts/financial_data.py](scripts/market_data.py) (用途与参数:获取财务/指标/估值数据，支持 --type/--code/--statDate/--columns)
